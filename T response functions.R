...
#fVc_opt <- as.formula(yvar~k25*((200*exp((Ea*(TsK-Topt))/TsK*0.008314*Topt))/(200-(Ea(1-exp((200*(TsK-Topt))/TsK*0.008314*Topt))))))
...
makecurves <- function(dir, fname) {
  ...
  fits1 <- fitacis(data, "Curve", fitmethod="bilinear", Tcorrect=FALSE,useRd=FALSE)
  fits2 <- fitacis(data, "Curve", fitTPU=TRUE, Tcorrect=FALSE)
  
  pdf(paste0(dir,"/TPU_ornot_p.pdf"), width=9, height=5)
  par(mfrow=c(1,2))
...
makedata<- function(path, fname, fit) {
  ...
  ret$A_ratio<-with(ret,highAs/lowAs)
...
get_summary_one<-function(data,from,to,by1){
  ...
  #labels=FALSE)
  Rday<-summaryBy(Vcmax+Jmax+JVr+lowAs+Rd+highAs+A_ratio~by1,data=data.1,FUN=c(mean,std.error),na.rm=T)
  
  tpuFrac<-function(){
    tpu_f<-function(data){apply(data, 2, function(x) length(which(!is.na(x))))[c(2,5)]}
    spl<-split(data,paste(data[[by1]]))
    get_tpu_frac<-get_topts(lapply(spl,FUN=tpu_f))
    
    get_tpu_frac[[by1]]<-names(spl)
    
    
    names(get_tpu_frac)[c(1:2)]<-c("nVcmax","nTPU")
    
    return(get_tpu_frac)
  }
  
  tpu<-tpuFrac()
  names(Rday)[1:15]<-c(by1,"Vc","Vj","JVr","Photo_275","Rday","Photo_800","A_ratio","Vc_SE","Vj_SE","JVr_SE","Photo_275_SE","Rday_SE","Photo_800_SE","A_ratio_SE")
  Rday<-merge(Rday,tpu,by1)
  return(Rday)
}

#get_summary_one(data=damz,by1="Season",from=23,to=27)
...
get_summary_two<-function(data,by1,by2,from,to){
  ...
  data.1<-data[data$Ts >= from & data$Ts <= to,]
  Rday<-summaryBy(Vcmax+Jmax+JVr+lowAs+Rd+highAs+A_ratio~by1+by2,data=data.1,FUN=c(mean,std.error),na.rm=TRUE)
  
  tpuFrac<-function(){
    tpu_f<-function(data){apply(data, 2, function(x) length(which(!is.na(x))))[c(2,5)]}
    spl<-split(data,paste(data[[by1]],data[[by2]]))
    get_tpu_frac<-get_topts(lapply(spl,FUN=tpu_f))
    
    get_tpu_frac[[by1]]<-get_names(spl)[,1]
    get_tpu_frac[[by2]]<-get_names(spl)[,2]
  ...
  names(Rday)[1:16]<-c(by1,by2,"Vc","Vj","JVr","Photo_275","Rday","Photo_800","A_ratio","Vc_SE","Vj_SE","JVr_SE","Photo_275_SE","Rday_SE","Photo_800_SE","A_ratio_SE")
  Rday<-merge(Rday,tpu,c(by1,by2))
  return(Rday)
}
...
#function to fit temperature response of vcmax (mix model)
...
read_and_convert_licor <- function(path){
  ...
  #function to fit gs model and get parameters and SEs
  #assume g0=0
